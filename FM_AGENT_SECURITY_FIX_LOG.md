...
| 38 | `b976fc79f6` | bazel solib 缺版本化 SONAME | SDK SONAME 是 `libfmsh_ukey_sdk.so.0`，但 cc_import 只把无版本 `libfmsh_ukey_sdk.so` 拷进 `_solib_k8`——bazel 构建的 codex NEEDED `.so.0` 运行时找不到（ldd not found）。修复：新增 `fmsh_ukey_sdk_soname_shared` cc_import（`shared_library = libfmsh_ukey_sdk.so.0`）使版本化文件也进 solib，RUNPATH 覆盖两个目录。验证：bazel codex ldd 全解析。另复核上游 `86fb662177`（consolidate guard/rpc/policy/license 进 fm crates）未旁路本分支修复：`guard_stdin_input`（write_stdin 调用点 + record_blocked 审计）、`process_write_stdin` TOCTOU guard、`recover_lock` 全部完好，fm-encrypted-skills 232/232 |
