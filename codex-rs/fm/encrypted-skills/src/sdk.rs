...
/// upstream semantics.
#[cfg(all(target_os = "linux", target_arch = "x86_64", target_env = "gnu", feature = "ukey"))]
pub struct AutoSdk {
...
pub fn sdk_for(kind: SdkKind) -> Arc<dyn EnvelopeSdk> {
    match kind {
        ...
        #[cfg(not(all(target_os = "linux", target_arch = "x86_64", target_env = "gnu", feature = "ukey")))]
        SdkKind::Auto { .. }
        | SdkKind::Software { .. }
        | SdkKind::UKey
        | SdkKind::UKeyTwoPhase { .. } => {
            tracing::warn!("encrypted-skill SDK kind is not compiled in (ukey feature off or unsupported platform)");
            Arc::new(UnavailableSdk)
        }
    }
}
