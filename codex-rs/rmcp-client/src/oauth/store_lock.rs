...
impl OAuthStoreLock {
    pub(super) fn acquire(store: OAuthStore) -> Result<Self, OAuthStoreLockFailure> {
        ...
        // authority. Those stores are GREVO_HOME-backed today: if GREVO_HOME is unset they use
...
pub(super) enum OAuthStoreLockFailure {
    #[error("failed to resolve GREVO_HOME for MCP OAuth {store} aggregate-store lock")]
    CodexHome {
        store: OAuthStore,
        #[source]
        source: io::Error,
