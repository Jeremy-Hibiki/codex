...
async fn import_plugins_supports_external_agent_plugin_marketplace_layout() {
    ...
    fs::create_dir_all(plugin_root.join(".grevo-plugin")).expect("create plugin manifest dir");
    ...
    fs::write(
        plugin_root.join(".grevo-plugin").join("plugin.json"),
...
async fn import_plugins_reuses_configured_marketplace_with_different_source() {
    ...
    fs::create_dir_all(configured_plugin_root.join(".grevo-plugin"))
    ...
    fs::create_dir_all(source_plugin_root.join(".grevo-plugin"))
...
"#
    ...
    fs::write(
        configured_plugin_root.join(".grevo-plugin/plugin.json"),
        r#"{"name":"cloudflare","version":"0.1.0"}"#,
    )
    .expect("write configured plugin manifest");
    fs::write(
        source_plugin_root.join(".grevo-plugin/plugin.json"),
...
async fn detect_repo_supports_project_relative_external_agent_plugin_marketplace_path() {
    ...
    let codex_home = root.path().join(".grevo");
