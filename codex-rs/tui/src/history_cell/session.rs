    ...
    if is_first_event {
        ...
        let help_lines: Vec<Line<'static>> = vec![
            ...
            Line::from(vec![
                "  ".into(),
                "/init".into(),
                " - create a GREVO.md file with instructions for Grevo".dim(),
            ]),
            Line::from(vec![
                "  ".into(),
                "/status".into(),
...
impl HistoryCell for SessionHeaderHistoryCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        // Title line rendered inside the box: ">_ Grevo (vX)"
        let title_spans: Vec<Span<'static>> = vec![
            Span::from(">_ ").dim(),
            Span::from("Grevo").bold(),
            Span::from(" ").dim(),
    ...
    fn raw_lines(&self) -> Vec<Line<'static>> {
        let mut lines = vec![
            Line::from(format!("Grevo (v{})", self.version)),
            Line::from(format!(
                "model: {}{}",
                self.model,
