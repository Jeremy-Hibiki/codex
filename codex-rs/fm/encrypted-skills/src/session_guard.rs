...
impl SessionGuard<'_> {
    ...
    /// Decrypted directories whose plaintext this session may execute from.
    ///
    /// Callers grant the sandbox read access to these so skill scripts run;
    /// every other use of the paths stays blocked by [`Self::before_tool`].
    pub fn guarded_read_dirs(&self) -> Vec<std::path::PathBuf> {
        self.runtime.decrypted_dirs(self.session_id())
    }

    /// Guard one tool invocation for this session.
    pub fn before_tool(&self, tool_name: &str, tool_input: &Value) -> GuardDecision {
        crate::guard::before_tool(self.runtime, self.session_id(), tool_name, tool_input)
    }
