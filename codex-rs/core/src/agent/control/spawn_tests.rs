...
fn user_message(text: &str) -> RolloutItem {
    RolloutItem::ResponseItem(codex_history::ResponseItemEnvelope::from(
...
fn assistant_final_answer_is_kept() {
    let item = RolloutItem::ResponseItem(codex_history::ResponseItemEnvelope::from(
        ResponseItem::Message {
            id: None,
...
fn strip_encrypted_skill_tokens_covers_response_item_text_surfaces() {
    ...
    let function_call = RolloutItem::ResponseItem(codex_history::ResponseItemEnvelope::from(
        ResponseItem::FunctionCall {
            id: None,
            call_id: "call-1".to_string(),
            name: "shell".to_string(),
    ...
    let custom_tool_call = RolloutItem::ResponseItem(codex_history::ResponseItemEnvelope::from(
    ...
    let function_output = RolloutItem::ResponseItem(codex_history::ResponseItemEnvelope::from(
    ...
    let custom_output = RolloutItem::ResponseItem(codex_history::ResponseItemEnvelope::from(
    ...
    let agent_message = RolloutItem::ResponseItem(codex_history::ResponseItemEnvelope::from(
    ...
    let reasoning = RolloutItem::ResponseItem(codex_history::ResponseItemEnvelope::from(
...
fn strip_encrypted_skill_tokens_covers_compacted_items() {
    let item = CompactedItem {
        ...
        replacement_history: Some(vec![codex_history::ResponseItemEnvelope::from(
            ResponseItem::Message {
                id: None,
                role: "user".to_string(),
                content: vec![ContentItem::InputText {
    ...
    let ResponseItem::Message { content, .. } =
        &*compacted.replacement_history.as_ref().unwrap()[0]
    else {
