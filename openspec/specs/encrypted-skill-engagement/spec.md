# encrypted-skill-engagement

## Purpose

定义“会话是否处于加密 Skill 环境”的统一判定：明文已存在或解密 in-flight 时 engaged，其余时间未 engaged；并提供 `(解密目录, 逻辑路径)` 映射导出，供沙箱 bind 与 RPC 面使用。
...
The engagement determination SHALL only introduce the engagement determination, in-flight tracking, and path-mapping export. It SHALL NOT change the behavior of existing guard or redaction rules for engaged sessions, and SHALL NOT apply any guard or redaction behavior to unengaged sessions.
...
#### Scenario: Unengaged sessions are not gated
- **WHEN** a session is not engaged
- **THEN** the engagement change introduces no guard or redaction behavior for that session
