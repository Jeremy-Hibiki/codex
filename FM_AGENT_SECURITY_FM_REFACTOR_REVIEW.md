...
| P4（R2/R5） | 部分完成 | R5 已落地：`Session::encrypted_skill_rehydrator()` 消除 compact/compact_remote/prompt_debug/turn 5 处重复构造（compact_remote 相关测试通过；`remote_compact_trim_estimate_uses_session_base_instructions` 在 stash 前后均失败，确认为环境相关、与改动无关）。R2（SessionGuard 句柄，约 25 处调用点）保持建议暂缓，待确认后作为独立批次实施。 |
