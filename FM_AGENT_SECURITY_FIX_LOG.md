...
| 23 | `053647880a` + `33f94b4371` | fmsh-ukey integration | Vendor `fmsh-ukey-wrapper`/`fmsh-ukey-cipher` at upstream `f09dc46` (openssl >= 0.10.76), add optional `fmsh-ukey` feature, implement `UKeySdk`/`LocalSdk`, wire `local_privkey` config; openssl lock bumped 0.10.75 -> 0.10.81; feature build verified with a stub SDK (`136 passed`) |
