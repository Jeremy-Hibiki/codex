...
| 37 | `待提交` | 上游 9c47981 升级 + 动态库运行时定位修复 | ① 同步上游 `9c47981`（大重构：`fmsh-ukey-cipher`/`fmsh-ukey-wrapper` 改名 `fmsh-ukey-core`/`fmsh-ukey-sdk-wrapper`，新增 `links = "fmsh_ukey_sdk"` + `scripts/bundle_sdk.sh`）：Cargo.toml 依赖名+rev、sdk.rs 24 处 crate 引用、MODULE.bazel annotation crate 名同步；API 完全兼容（Cipher/KeyWrap/SoftwareCipher/UkeyCipher/UkeyTwoPhaseCipher/SoftwareAlgorithm 均在），测试 148→**232/232** 全绿；② 修运行时动态库解析：cargo 构建的 codex 二进制 NEEDED SDK 但无 RPATH（rustc-link-arg 不传播，上游注释确认），bazel 构建的 codex RUNPATH 指向 solib 但 `libcrypto.so.1.1` 不在（DT_RUNPATH 不传递）——**codex-cli/build.rs** 加 `$ORIGIN/lib` rpath + `--no-as-needed -l:libcrypto.so.1.1` 强制 NEEDED（对齐上游 enc 方案），**additive.BUILD.bazel** 加 `fmsh_ukey_crypto_shared` cc_import 让 libcrypto 进 solib；验证：cargo+bazel 构建的 codex ldd 全解析，干净环境（env -i + $ORIGIN/lib）部署场景通过 |
