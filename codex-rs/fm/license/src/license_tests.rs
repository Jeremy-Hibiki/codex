...
fn heartbeat_retry_success_does_not_rearm_compensation_checkout() {
    // `on_retry_success` mutates the process-global license state, so this
    // test must take the same lock as every other state-touching test.
    let _state_lock = LICENSE_STATE_TEST_LOCK
        .lock()
        .unwrap_or_else(std::sync::PoisonError::into_inner);
...
fn heartbeat_loss_after_shutdown_does_not_attempt_checkout() {
    ...
    // Restore the active state: the lock serializes state-touching tests but
    // does not reset the process-global between them.
    mark_license_active();
}
