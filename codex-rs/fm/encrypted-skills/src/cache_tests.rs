...
fn store_returns_distinct_tokens_for_distinct_plaintext() {
    ...
    let a = cache.store("s1", content("alpha"));
    let b = cache.store("s1", content("beta"));
    assert_ne!(a, b);
    assert_eq!(
...
fn store_deduplicates_identical_plaintext() {
    let mut cache = ContentCache::new(64);
    let a = cache.store("s1", content("same"));
    let b = cache.store("s1", content("same"));
    assert_eq!(a, b);
}
...
fn sessions_are_isolated() {
    let mut cache = ContentCache::new(64);
    let a = cache.store("s1", content("text"));
    assert_eq!(
        cache.lookup("s1", &a).map(|c| c.plaintext.as_str()),
        Some("text")
    );
...
fn fifo_eviction_respects_cap() {
    ...
    let a = cache.store("s1", content("one"));
    let b = cache.store("s1", content("two"));
    let c = cache.store("s1", content("three"));
...
fn plaintexts_iterates_session_entries() {
    let mut cache = ContentCache::new(64);
    cache.store("s1", content("one"));
    cache.store("s1", content("two"));
    cache.store("s2", content("other"));
...
fn total_size_cap_evicts_oldest() {
    ...
    let a = cache.store("s1", content("aaaaaa")); // 6 bytes
    let b = cache.store("s1", content("bbbbbb")); // 12 bytes total -> evicts a
    let c = cache.store("s1", content("cccccc")); // 12 bytes total -> evicts b
    assert_eq!(cache.lookup("s1", &a), None);
