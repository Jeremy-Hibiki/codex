...
pub struct TestAppServer {
    ...
    // removing an owned GREVO_HOME that may still be its cwd on Windows.
...
impl TestAppServer {
    /// Starts building a server with a temporary GREVO_HOME and the standard
...
impl TestAppServerBuilder {
    /// Uses this existing GREVO_HOME instead of a temporary one.
    ...
    /// Builds a server with a temporary GREVO_HOME and automatic environment
    /// by default.
