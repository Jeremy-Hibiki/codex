...
async fn import_repo_hooks_preserves_disabled_codex_hooks_feature() {
    ...
    fs::create_dir_all(repo_root.join(EXTERNAL_AGENT_DIR)).expect("create external agent dir");
    fs::create_dir_all(repo_root.join(".grevo")).expect("create codex dir");
    fs::write(
    ...
    .expect("write hooks");
    fs::write(
        repo_root.join(".grevo").join("config.toml"),
        "[features]\ncodex_hooks = false\n",
    )
    .expect("write config");
    ...
    assert_eq!(
        fs::read_to_string(repo_root.join(".grevo").join("config.toml")).expect("read config"),
        "[features]\ncodex_hooks = false\n"
    );
    let hooks: JsonValue = serde_json::from_str(
        &fs::read_to_string(repo_root.join(".grevo").join("hooks.json")).expect("read hooks"),
    )
    .expect("parse hooks");
    assert_eq!(
        hooks,
...
async fn import_repo_mcp_uses_home_settings_toggles_when_repo_settings_missing() {
    ...
    let outcome = service_for_paths(external_agent_home, root.path().join(".grevo"))
        .import(vec![ExternalAgentConfigMigrationItem {
...
async fn import_repo_mcp_uses_local_settings_toggles_over_project_settings() {
    ...
    service_for_paths(external_agent_home, root.path().join(".grevo"))
        .import(vec![ExternalAgentConfigMigrationItem {
            item_type: ExternalAgentConfigMigrationItemType::McpServerConfig,
            description: String::new(),
            cwd: Some(repo_root.clone()),
