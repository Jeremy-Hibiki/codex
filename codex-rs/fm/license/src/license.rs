...
pub fn install_checkin_signal_handler() -> Result<()> {
    ...
    std::thread::spawn(move || {
        // Check the license in as soon as a termination signal arrives, then
        // let the host perform its own shutdown. Hosts handle these signals
        // themselves (the app-server drains in-flight turns, the TUI restores
        // the terminal), and their `LicenseGuard` drop is idempotent — the
        // check-in already happened here. Exiting from this thread would
        // preempt that drain and kill the process mid-shutdown.
        while signals.forever().next().is_some() {
            check_in_now();
        }
    });
    Ok(())
