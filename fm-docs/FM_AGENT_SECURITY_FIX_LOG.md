...
| 36 | `04cbbe29ec` | 两阶段密钥缓存内容寻址 + fmsh-ukey 硬绑定 | ① `UkeyTwoPhaseSdk.ciphers` 缓存键从 `PathBuf`（key_path）改为 `Vec<u8>`（key.enc 文件内容本身）——N 个 Skill 共享同一份 key.enc 时，unwrap 从 N 次收敛为 1 次，后续全部命中同一内存 cipher；独立密钥 Skill 不受影响；零额外依赖（文件内容即键）。新增 `shared_key_envelope_is_unwrapped_exactly_once_across_skills`（counting KeyWrap 断言两个目录同一 key.enc 只 unwrap 一次）；② 按部署要求将 fmsh-ukey-cipher 从 `optional` feature 改为**硬绑定**（`[target.'cfg(linux x86_64 gnu)'.dependencies]` 非 optional），删除 `fmsh-ukey` feature 定义与 sdk.rs/config_toml.rs 全部 `feature = "fmsh-ukey"` cfg（改为纯 target 门控）；默认构建即含 software/ukey/ukey-two-phase 三个真实后端（147/147 默认跑通，含 HPKE/两阶段/共享密钥真实解密）；非 Linux 平台仍 fail-closed（vendored `.so` 仅 Linux x86_64） |
| 37 | `93708c8dc8` | 上游 9c47981 升级 + 动态库运行时定位修复 | ① 同步上游 `9c47981`（大重构：`fmsh-ukey-cipher`/`fmsh-ukey-wrapper` 改名 `fmsh-ukey-core`/`fmsh-ukey-sdk-wrapper`，新增 `links = "fmsh_ukey_sdk"` + `scripts/bundle_sdk.sh`）：Cargo.toml 依赖名+rev、sdk.rs 24 处 crate 引用、MODULE.bazel annotation crate 名同步；API 完全兼容（Cipher/KeyWrap/SoftwareCipher/UkeyCipher/UkeyTwoPhaseCipher/SoftwareAlgorithm 均在），测试 148→**232/232** 全绿；② 修运行时动态库解析：cargo 构建的 codex 二进制 NEEDED SDK 但无 RPATH（rustc-link-arg 不传播，上游注释确认），bazel 构建的 codex RUNPATH 指向 solib 但 `libcrypto.so.1.1` 不在（DT_RUNPATH 不传递）——**codex-cli/build.rs** 加 `$ORIGIN/lib` rpath + `--no-as-needed -l:libcrypto.so.1.1` 强制 NEEDED（对齐上游 enc 方案），**additive.BUILD.bazel** 加 `fmsh_ukey_crypto_shared` cc_import 让 libcrypto 进 solib；验证：cargo+bazel 构建的 codex ldd 全解析，干净环境（env -i + $ORIGIN/lib）部署场景通过 |
| 38 | `b976fc79f6` + `696e2380f5` | bazel solib 缺版本化 SONAME + libcrypto 非 NEEDED | ① SDK SONAME 是 `libfmsh_ukey_sdk.so.0`，但 cc_import 只把无版本 `libfmsh_ukey_sdk.so` 拷进 `_solib_k8`——bazel codex NEEDED `.so.0` 运行时找不到。修复：`fmsh_ukey_sdk_soname_shared` cc_import（`shared_library = libfmsh_ukey_sdk.so.0`）让版本化文件进 solib。② libcrypto.so.1.1 是 SDK 的依赖，主二进制无直接符号引用 → `--as-needed` 把它从最终链接丢弃，DT_RUNPATH 不传递导致 SDK 的 NEEDED libcrypto 永远无法解析（此前"验证通过"是 LD_LIBRARY_PATH 污染的假象）。修复：`fmsh_ukey_native` 加 `linkopts = ["-Wl,--no-as-needed", "-l:libcrypto.so.1.1", "-Wl,--as-needed"]` 强制主二进制 NEEDED libcrypto（对齐 cargo build.rs 方案）。验证：`env -i` 干净环境（无 LD_LIBRARY_PATH）下 bazel codex 的 ldd 全解析两个库。另复核上游 `86fb662177`（consolidate guard/rpc/policy/license 进 fm crates）未旁路本分支修复：`guard_stdin_input`（write_stdin 调用点 + record_blocked 审计）、`process_write_stdin` TOCTOU guard、`recover_lock` 全部完好，fm-encrypted-skills 232/232 |
