...
pub unsafe fn protect_workspace_codex_dir(cwd: &Path, psid: *mut c_void) -> Result<bool> {
    let grevo_protected = protect_workspace_subdir(cwd, psid, ".grevo")?;
    let legacy_protected = protect_workspace_subdir(cwd, psid, ".codex")?;
    Ok(grevo_protected || legacy_protected)
}
