...
//! this crate checks out a license from the FMSH LicenseService before Grevo
//! starts and holds it for the lifetime of the process. Everywhere else the
//! crate compiles to an empty stub: the gate passes every request through
//! (baseline behavior) and no internal GitLab dependency is needed to
//! build. The stub ignores the `FMSH_CODEX_LIC_TEST_*` env vars.
//!
...
#[cfg(all(target_os = "linux", target_arch = "x86_64", target_env = "gnu", feature = "lmclient"))]
mod license;
...
#[cfg(not(all(target_os = "linux", target_arch = "x86_64", target_env = "gnu", feature = "lmclient")))]
...
#[cfg(any(
    test,
    not(all(target_os = "linux", target_arch = "x86_64", target_env = "gnu", feature = "lmclient"))
))]
